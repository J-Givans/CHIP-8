...
namespace chip8
{
        ...
        while (window_.isOpen()) {
            ...
            while (window_.pollEvent(event)) {
                if (event.type == sf::Event::Closed) {
                    ...
                    handleOnKeyPressedEvents(keys); 
                }
                else if (event.type == sf::Event::KeyReleased) {
                    handleOnKeyReleasedEvents(keys);
                }
            }
        }
    }
    
    void Platform::handleOnKeyPressedEvents(std::array<uint8_t, 16>& keys)
    {
        using sf::Keyboard;
    ...
    void Platform::handleOnKeyReleasedEvents(std::array<uint8_t, 16>& keys) const
    {
        using sf::Keyboard;
